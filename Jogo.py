...
class Jogo:
    ...
    def getX(self, i, j):
        if i == 0 or i == 2 or i == 5 or i == 7 or i==3:
            if j == 1:
                return 65
            if j == 3:
                return 195
    ...
    def movimenta_peca(self, screen, mouse_pos, board): # mouse_pos guarda as posicoes antiga e nova do mouse num array contendo 2 tuplas
        ...
        for i in range(8):
            if limite_posicoes[i] > mouse_pos[1][1]:
                ...
                break
        # Corrigir depois para o jogo real
        if self.matriz_tabuleiro[indice_antigo_superior_y][indice_antigo_superior_x] == 'a' and self.matriz_tabuleiro[indice_novo_superior_y][indice_novo_superior_x] == '_':
            self.matriz_tabuleiro[indice_antigo_superior_y][indice_antigo_superior_x] = '_'
            self.matriz_tabuleiro[indice_novo_superior_y][indice_novo_superior_x] = 'a'
            screen.fill((0,0,0))
